...
class Music(commands.Cog):
    ...
    @commands.command(name='войди', invoke_without_subcommand=True)
    ...
    @commands.command(name='выйди', aliases=['disconnect'])
    ...
    async def _volume(self, ctx: commands.Context, *, volume: int):
        ...
        discord.PCMVolumeTransformer(original=YTDLSource, volume=volume)
        await ctx.send('Громкость изменена на {}%'.format(volume))

    @commands.command(name='сейчас', aliases=['current', 'playing'])
    ...
    @commands.command(name='перемешать')
...
bot = commands.Bot(command_prefix='-', description='Крутой бот от никоскокоса).')
